  ...
  return (
    <>
      ...
      <Card>
        ...
        <p className="mt-2 text-xs text-muted">Flag toggles are audited via the debug log.</p>
      </Card>
      ...
      <Card>
        ...
        {jobs.length === 0 ? <p className="text-sm text-muted">No jobs recorded yet.</p> : null}
      </Card>
            ...
            {logs.map((l) => (
              <li key={l.id} className="rounded-lg border border-border p-3 text-sm">
                ...
                {Object.keys((l.metadata as object) ?? {}).length > 0 ? (
                  <pre className="mt-2 overflow-x-auto rounded bg-bg p-2 text-xs text-muted">
