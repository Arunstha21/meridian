        ...
        {(["expense", "income", "transfer"] as const).map((m) => (
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-medium capitalize ${
              mode === m ? "bg-primary/10 text-primary" : "text-muted hover:bg-border/40"
            }`}
          >
            {m}
          </button>
    ...
    <Card>
      <form action={action} className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Description" htmlFor="name">
          <Input id="name" name="name" required maxLength={240} placeholder={mode === "expense" ? "Grocery run" : "Salary"} />
        </Field>
...
function TransferForm({ accounts, presetAccount }: { accounts: AccountOption[]; presetAccount: string }) {
  const [state, action] = useActionState(createTransferAction, undefined);
  const today = new Date().toISOString().slice(0, 10);
  const sameCurrency = accounts.filter(
    (a) => !presetAccount || a.currency === accounts.find((x) => x.id === presetAccount)?.currency || a.id !== presetAccount
  );
  void sameCurrency;
  return (
    <Card>
      <form action={action} className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label="From account" htmlFor="fromAccountId">
          <Select id="fromAccountId" name="fromAccountId" defaultValue={presetAccount || accounts[0]?.id} required>
            {accounts.map((a) => (
              <option key={a.id} value={a.id}>{a.name}</option>
            ))}
          </Select>
        </Field>
        <Field label="To account" htmlFor="toAccountId" hint="Accounts must share the same currency.">
        ...
        <div className="sm:col-span-2">
          <Field label="Note (optional)" htmlFor="t-name">
            <Input id="t-name" name="name" maxLength={240} placeholder="Monthly credit card payment" />
          </Field>
        </div>
        <div className="sm:col-span-2">
